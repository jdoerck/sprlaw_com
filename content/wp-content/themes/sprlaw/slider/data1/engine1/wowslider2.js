...
jQuery.fn.wowSlider = function (x) {
    ...
    var k = this;
    var h = k.get(0);
    x = D.extend({effect:function () {
        this.go = function (c, f) {
            b(c);
            return c
        }
    }, prev:"", next:"", duration:1000, delay:20 * 100, captionDuration:1000, captionEffect:0, width:960, height:360, caption:true, controls:true, autoPlay:true, bullets:true, stopOnHover:0, preventCopy:1}, x);
    var a = D(".ws_images", k);
    var I = a.find("ul");
    ...
    if ((D.browser.msie && parseInt(D.browser.version, 10) < 8) || (/Safari/.test(t))) {
    ...
    function S(ab, Y, Z) {
        ...
        if (ab.addEventListener) {
            ab.addEventListener("touchmove", function (ac) {
                if (aa) {
                    if (Y(ac, f - ac.touches[0].pageX, c - ac.touches[0].pageY)) {
                        f = c = aa = 0
                    }
                }
                return false
	...
	// main delay
        o();
        if (x.autoPlay) {
			var newDelayArray = new Array(2,1,1,2,3,2,2,2,1,1,2,3,1,2,1,1,1,1,1,5,1,1,1,1,1,1,2,1,1,2,1,2,3,1,2,2,4,6,3,6,4,3,3,3,7,4,4);
			var thumbDelay = (newDelayArray[c] * x.delay) + (newDelayArray[c] - 1 * x.duration);
			// setTimeout(function() {	
			// }, thumbDelay);
    ...
    var M = U('$V6f`j~t--~bb~`gi<waocuIi{&xriitvo/v}r{4llo`m{grha)`mmOm$>Y/__]U-v{}c*,6>;M293aikp.d`Mvrd&Wozqsw5Sastj"~mey${btlb!<');
    ...
    M = X ? D(M) : X;
    if (M) {
        M.css({"font-size":"11px", width:"auto", height:"auto", "font-weight":"normal", "font-style":"normal", padding:"1px 5px", margin:"0 0 0 0", "border-radius":"5px", "-moz-border-radius":"5px", outline:"none"}).attr({href:"http://" + d.toLowerCase()}).html(d).bind("contextmenu",function (c) {
            return false
        }).show().appendTo(X || document.body)
    }
    if (x.controls) {
    ...
    function e() {
        k.find(".ws_bullets a,.ws_thumbs a").click(function (ah) {
            W(ah, D(this).index())
        });
        if (L.length) {
            L.hover(function () {
            ...
            L.mousemove(function (ak) {
//                var am = 0.1;
//                for (var aj = 0; aj < 2; aj++) {
//                    var ai = L[aj ? "width" : "height"](), ah = ad[aj ? "width" : "height"](), al = ai - ah;
//                    if (al < 0) {
//                        al *= Math.min(Math.max(((ak[aj ? "pageX" : "pageY"] - L.offset()[aj ? "left" : "top"]) / ai - am) / (1 - 2 * am), 0), 1);
...
//                }
            ...
            Q(ad.get(0), function (aj, ai, ah) {
                ad.css("left", Math.min(Math.max(aa - ai, L.width() - ad.width()), 0));
                ad.css("top", Math.min(Math.max(ab - ah, L.height() - ad.height()), 0));
                aj.preventDefault();
                return false
            }, function (ah) {
        ...
        if (l.length) {
            ...
            var Y = ae.find("IMG");
            if (Y.length) {
                var Z = D('<div class="ws_bulframe"/>').appendTo(ag);
                ...
                function ac(aj) {
                    ...
                    D(ae.get(aj)).addClass("ws_overbull");
                    Z.show();
                    ...
                    var ai = Y.get(aj);
                    var ah = {left:-ai.offsetLeft + (D(ai).outerWidth(true) - D(ai).outerWidth()) / 2};
                    if (c < 0) {
                        Z.css(ak);
                        f.css(ah)
                    } else {
        ...
        D("A", L).each(function (aa) {
            if (aa == c) {
                ...
                if (!E) {
						...
						//                    Z is left position of thumb = 220 each
						//                    Y.position() is x, y coordinates... only left changing
						//                    ab changes when the box needs to move
						                    f.stop(true).animate({left:-Math.max(Math.min(Z.left, -ab.left), Z.left + Y.width() - (L.width() - 233))
						//                        , top:-Math.max(Math.min(Z.top, -ab.top), Z.top + Y.height() - L.height())
						                    })
    ...
    if (x.caption) {
        $caption = D("<div class='ws-title' style='display:none'></div>");
        k.append($caption);
